...
void CNC::GoToOrigen(bool X, bool Y, bool Z)
{
  ...
  RegulaVentDrivers();
  //Separo el plano Z;
  PAPMotorZ.SetSteps(_PasosSepTraslacion, 1);
  PAPMotorZ.AllSteps();
...
void CNC::DefineDestino(float X, float Y, float Z, byte Modo)
{
  if(PAPMotorX.GetModo() != Modo)
  {
    //Si cambia de impresión 1 o 2 a traslación, Baja el plato 400 pasos
	if((PAPMotorX.GetModo()==2 || PAPMotorX.GetModo()==4 || PAPMotorX.GetModo()==8) && (Modo==1))
	{	  
	  PAPMotorE.SetSteps(-_PasosExtTraslacion, (byte)1);//recoge filamento
      PAPMotorE.AllSteps();
	  
      PAPMotorZ.SetSteps(-_PasosSepTraslacion, (byte)1);//baja plataforma
      PAPMotorZ.AllSteps();
	  ...
	  //Falta liquidar el resto de Pasos; sobraría el resto
	  PAPMotorZ.SetSteps(_RestoPasosZ, PAPMotorZ.GetModo());
	
	  Run(true);
	}
	//Si cambia de traslación a impresión 1 o 2, Sube el plato 400 pasos
	else if((Modo==2 || Modo==4 || Modo==8) && (PAPMotorX.GetModo()==1))
	{	  
	  //Falta liquidar el resto de Pasos
      PAPMotorZ.SetSteps(_PasosSepTraslacion, (byte)1);//sube plataforma
      PAPMotorZ.AllSteps();
	  PAPMotorE.SetSteps(_PasosExtTraslacion, (byte)1);//extrusiona
      PAPMotorE.AllSteps();
	}
  }
  ...
  long Pasos = (long)((X - _CurrentX) / MicropasoX);//micropasos hasta la posición más cercana
  ...
  Pasos = (long)((Y - _CurrentY) / MicropasoY);//micropasos hasta la posición más cercana
  ...
  Pasos = (long)((Z - _CurrentZ) / MicropasoZ);//micropasos hasta la posición más cercana
  PAPMotorZ.SetSteps(Pasos, (byte)1);
...
void CNC::Run(bool Resto)
{
  ...
  if (Resto || PAPMotorX.GetModo() == (byte)1)
  {
    //Serial.println("Modo sin extrusión");
	//Sin extrusión		
		//Pasos y Modo en Y
  ...
  {
    //Serial.println("Modo con extrusión");
	//Con extrusión
		float ContX = 0;
		float ContY = 0;
		float ContZ = 0;
		...
		for(long i = 1; i <= StepsMax; i++)
		{
		  ...
		  //noInterrupts();
		  {
			...
			if(PAPMotorX.RemainSteps() != 0)
			{
		      ...
		      DestX = TempX - ContX;//Distancia desde la posición actual
		      ContX = TempX;//Posición anterior
			}
			else
			{
			...
			if(PAPMotorY.RemainSteps() != 0)
			{
		      ...
		      DestY = (long)(TempY - ContY);//Distancia desde la posición actual
		      ContY = TempY;//Posición anterior
			}
			else
			{
			...
			if(PAPMotorZ.RemainSteps() != 0)
			{
		      ...
		      DestZ = (long)(TempZ - ContZ);//Distancia desde la posición actual
		      ContZ = TempZ;//Posición anterior
			}
			else
			{
		  ...
		  if(((i-1) % (long)PAPMotorX.GetModo()) == 0)
		  {
			...
			PAPMotorE.SetSteps((long)1, (byte)2);
            PAPMotorE.AllSteps();
		  }
...
{
  //Serial.println("INFInicio Regula Ventilación Drivers PAP");
  float TempDrivers = -1;
  ...
  //Serial.println("INFFin Regula Ventilación Drivers PAP");
}

void CNC::RegulaVentExtrusor(void)
{
  //Serial.println("INFInicio Regula Ventilación Extrusor");
  ...
  //Serial.println("INFFin Regula Ventilación Extrusor");
}
